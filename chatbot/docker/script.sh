...
sed -i /chatbot/app.js -e "s/<Customer_demo_url>/"$SERVICE_URL"/g"
sed -i /chatbot/app.js -e "s/<Customer_Store>/"$SCOPE"/g"
sed -i /chatbot/app.js -e "s/<link_to_images>/"$S3_URL"/g"
...
sed -i /chatbot/app.js -e "s/<AUTHENTICATION_LOGIN_URL>/https:\/\/"$SERVICE_URL"\/auth/g"
